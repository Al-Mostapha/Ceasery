...
$PlayerInData = selectFromTable("id_player", "player", "id_user = :idu", ["idu" => $OuthUser[0]["id_user"]]);

$playerId = 0 ;
if(count($PlayerInData))
    $playerId = validateId($PlayerInData[0]["id_player"]);


if(count($PlayerInData) > 1){
    header("Location: AskPrimary.php?ot={$_GET["ot"]}&server={$_GET["server"]}");
    exit();
}

$newToken = rand(1, 1e6)."-".rand(1, 1e6)."-".rand(1, 1e6)."-".rand(1, 1e6)."-".rand(1, 1e6);

if(count(selectFromTable("*", "player_auth", "id_player = :idp", ["idp" => $playerId])) > 0){
    updateTable("auth_token = :at", "player_auth", "id_player = :idp", ["idp" => $playerId, "at" => $newToken]);
}else{
    insertIntoTable("id_player = :idp, auth_token = :at, panned = (SELECT player.panned FROM player WHERE player.id_player = :idp2)", "player_auth", ["idp" => $playerId, "at" => $newToken, "idp2" => $playerId]);
}
...
if(existInTable("panned_ip", "ipv4 = :i", ["i" => $ipAdd]) == 1){
    exit("<html>"
            . "<body>"
            . "<script>alert('هذا ال ip محظور من قبل الادارة ')</script>"
            . "</body>"
            . "</html>");
}

$p_server = validateId($_GET["server"]);


if ($playerId == 0) {
    $String = sendPostReqCurl([
        "idUser" => $OuthUser[0]["id_user"],
        "outhToken" => $newToken,
        "server" => $p_server
    ], API_URL."/home/HLogIn/AddPlayer");
    
    $JsonString = json_decode($String);
    $playerId = $JsonString["idPlayer"];
...
$panTime = selectFromTable("*", "player", "id_player = :idp", ["idp" => $playerId]);

if($panTime[0]["panned"] > time()){

    die("<html>"
            . "<body>"
            . "<script>alert('هذا الحساب محظور من قبل الادارة ')</script>"
            . "</body>"
            . "</html>");
    exit();

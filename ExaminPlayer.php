...
$OuthAdmin = selectFromTableIndex("user_auth.*, game_user.user_group", "user_auth JOIN game_user ON user_auth.id_user = game_user.id_user", " user_auth.auth_token  = :ot", ["ot" => $_GET["AdminToken"]]);


if (!count($OuthAdmin))
    exit("لست عضو من الادارة");
if ($OuthAdmin[0]["user_group"] < 6)
    exit(" 4لست عضو من الادارة");


//$UserOuth = urldecode($_GET["ot"]);
$OuthUser = selectFromTableIndex("*", "user_auth", " id_user = :d", ["d" => $_GET["idPlayer"]]);
...
$playerId = validateId($_GET["idPlayer"]);
$token = selectFromTable("auth_token", "player_auth", "id_player = :idp", ["idp" => $_GET["idPlayer"]]);
if (!count($token)) {
    $newToken = passEnc($_GET["idPlayer"] . time() . rand(1, 5e6));
    queryExe("INSERT INTO player_auth(id_player, auth_token) VALUES (:idp, :t) ON DUPLICATE KEY UPDATE auth_token = :ot", ["idp" => $OuthUser[0]["id_user"], "t" => $newToken, "ot" => $newToken]);
} else {
